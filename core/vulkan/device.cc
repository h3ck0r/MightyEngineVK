...
Device::Device(const vk::PhysicalDevice& physical_device,
    const QueueFamily& queue_family) {
    CreateDevice(physical_device, queue_family.queue_family_index());
}
...
void Device::CreateDevice(const vk::PhysicalDevice& physical_device,
    ...
    vk::PhysicalDeviceRayTracingPipelineFeaturesKHR ray_tracing_pipeline_features{true};
    ...
    device_ =
        physical_device.createDeviceUnique(createInfoChain.get<vk::DeviceCreateInfo>());
    VULKAN_HPP_DEFAULT_DISPATCHER.init(*device_);
}

bool Device::CheckDeviceExtensionSupport(const vk::PhysicalDevice& physical_device,
    const std::vector<const char*>& required_extensions) const {
    std::vector<vk::ExtensionProperties> available_extensions =
        physical_device.enumerateDeviceExtensionProperties();
    std::vector<std::string> required_extension_names(required_extensions.begin(),
        required_extensions.end());

    for (const auto& extension : available_extensions) {
        required_extension_names.erase(std::remove(required_extension_names.begin(),
